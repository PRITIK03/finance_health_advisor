...
with col1_header:
    st.markdown("<div style='font-size: 4rem; text-align: center;'>≡ƒÆ░</div>", unsafe_allow_html=True)
with col2_header:
...
with st.sidebar:
    ...
    st.markdown("<div style='font-size: 3.5rem; text-align: center;'>≡ƒÆ░</div>", unsafe_allow_html=True)
    ...
    page = st.radio(
        "Go to section:",
        ["≡ƒôè Dashboard Overview", "≡ƒôê Data Visualization Dashboard", "≡ƒôÜ Financial Tips & Education", "≡ƒùé∩╕Å Document Upload & Storage", "≡ƒÜ¿ Stress Test", "≡ƒæÑ Comparison Mode", "≡ƒæÑ User Segmentation", "≡ƒÄ» Risk Prediction", 
         "≡ƒôê Forecasting", "≡ƒÜ¿ Anomaly Detection", "≡ƒÆí Recommendations", "≡ƒÆ╕ Expense Categorization", "≡ƒÄ» Goal Planner", "≡ƒö« Predictive Analytics", "≡ƒöö Alerts & Notifications", "≡ƒÜÇ Wealth Projection", "≡ƒöÑ FIRE Tracker", "≡ƒÆ╕ Debt Optimizer", "≡ƒæÑ Peer Benchmarking", "≡ƒö« Scenario Simulator", "≡ƒ¢á∩╕Å What-If Simulator", "≡ƒÆ╝ Tax Optimizer", "≡ƒôë Monte Carlo Simulator", "≡ƒô▒ Subscription Manager", "≡ƒÆ░ Net Worth Tracker", "≡ƒöì Data Explorer"],
...
# ============ COMPARISON MODE ============
    ...
    if u1_id == u2_id:
        ...
        st.markdown("<br>", unsafe_allow_html=True)
        with st.container(border=True):
            st.markdown("<p class='card-title'>≡ƒÅü Head-to-Head Summary</p>", unsafe_allow_html=True)
        ...
        with col3:
            with st.container(border=True):
                st.markdown("<p class='card-title'>≡ƒÄ» Comparative Radar Analysis</p>", unsafe_allow_html=True)
        ...
        with col4:
            with st.container(border=True):
                st.markdown("<p class='card-title'>≡ƒôè Metric Gap View</p>", unsafe_allow_html=True)
                st.plotly_chart(visualizer.create_user_comparison_chart(u1_data, u2_data), use_container_width=True)

        with st.container(border=True):
            st.markdown("<p class='card-title'>≡ƒôï Detailed Comparison Table</p>", unsafe_allow_html=True)
            st.dataframe(pd.DataFrame(comparison_rows), use_container_width=True)

# ============ USER SEGMENTATION ============
elif page == "≡ƒæÑ User Segmentation":
    ...
    tab1, tab2 = st.tabs(["≡ƒôè Cluster Overview", "≡ƒôï Detailed Statistics"])
    ...
    with tab1:
        ...
        with col1:
            with st.container(border=True):
                st.markdown("<p class='card-title'>≡ƒÄ» K-Means Cluster Profiles</p>", unsafe_allow_html=True)
                st.plotly_chart(visualizer.create_cluster_analysis_plot(), use_container_width=True)
            
        with col2:
            with st.container(border=True):
                st.markdown("<p class='card-title'>≡ƒæÑ Distribution</p>", unsafe_allow_html=True)
    ...
    with tab2:
        with st.container(border=True):
            st.markdown("<p class='card-title'>≡ƒôï Cluster Financial Averages</p>", unsafe_allow_html=True)
...
# ============ RISK PREDICTION ============
    ...
    tab1, tab2 = st.tabs(["≡ƒÄ» Prediction Insights", "≡ƒôè Importance & Distribution"])
    
    with tab1:
        col1, col2 = st.columns([1, 1])
        with col1:
            with st.container(border=True):
                st.markdown("<p class='card-title'>≡ƒÄ» Risk Category Distribution</p>", unsafe_allow_html=True)
        ...
        with col2:
            with st.container(border=True):
                st.markdown("<p class='card-title'>≡ƒºá Risk Intelligence</p>", unsafe_allow_html=True)
                st.markdown(f"""
                <div class='sidebar-text'>
    ...
    with tab2:
        with st.container(border=True):
            st.markdown("<p class='card-title'>≡ƒôè Model Feature Importance</p>", unsafe_allow_html=True)
...
# ============ FORECASTING ============
    ...
    tab1, tab2 = st.tabs(["≡ƒôê Trends", "≡ƒôè Segment Analysis"])
    
    with tab1:
        with st.container(border=True):
            st.markdown("<p class='card-title'>≡ƒôê Monthly Savings Trends & Projection</p>", unsafe_allow_html=True)
    ...
    with tab2:
        with st.container(border=True):
            st.markdown("<p class='card-title'>≡ƒôè Savings Performance by Risk Profile</p>", unsafe_allow_html=True)
...
# ============ ANOMALY DETECTION ============
elif page == "≡ƒÜ¿ Anomaly Detection":
    ...
    tab1, tab2 = st.tabs(["≡ƒÜ¿ Visual Analysis", "≡ƒôï Detected Anomalies"])
    
    with tab1:
        with st.container(border=True):
            st.markdown("<p class='card-title'>≡ƒÜ¿ Spending & Savings Anomalies</p>", unsafe_allow_html=True)
            st.plotly_chart(visualizer.create_anomaly_analysis_plot(), use_container_width=True)
        
    with tab2:
        with st.container(border=True):
            st.markdown("<p class='card-title'>≡ƒöì Flagged Transactions (AI Insights)</p>", unsafe_allow_html=True)
...
# ============ RECOMMENDATIONS ============
    ...
    rec_type = st.radio(
        ...
        ["≡ƒæñ Individual User", "≡ƒæÑ Cohort Analysis"],
        horizontal=True
    )
    
    if rec_type == "≡ƒæñ Individual User":
        # User selector
        ...
        tab1, tab2, tab3, tab4 = st.tabs(["≡ƒÆ░ Budget", "≡ƒÆ│ Debt", "≡ƒÅª Savings", "≡ƒôê Investments"])
        
        with tab1:
            with st.container(border=True):
                st.markdown("<p class='card-title'>≡ƒô¥ Budget Recommendations</p>", unsafe_allow_html=True)
                for rec in user_recs['budget']:
                    status_color = {'good': '≡ƒƒó', 'warning': '≡ƒƒí', 'critical': '≡ƒö┤', 'moderate': '≡ƒƒá'}.get(rec['status'], 'ΓÜ¬')
                    st.markdown(f"**{status_color} {rec['category']}**: {rec['message']}")
                    st.markdown(f"   *≡ƒÆí {rec['suggestion']}*")
                    st.markdown("")
        
        with tab2:
            with st.container(border=True):
                st.markdown("<p class='card-title'>≡ƒÆ│ Debt Management</p>", unsafe_allow_html=True)
                for rec in user_recs['debt']:
                    status_color = {'good': '≡ƒƒó', 'warning': '≡ƒƒí', 'critical': '≡ƒö┤', 'moderate': '≡ƒƒá', 'excellent': '≡ƒÆÜ', 'info': '≡ƒö╡'}.get(rec['status'], 'ΓÜ¬')
                    st.markdown(f"**{status_color} {rec['category']}**: {rec['message']}")
                    st.markdown(f"   *≡ƒÆí {rec['suggestion']}*")
                    st.markdown("")
        ...
        with tab3:
            with st.container(border=True):
                st.markdown("<p class='card-title'>≡ƒÅª Savings & Safety Net</p>", unsafe_allow_html=True)
                for rec in user_recs['savings']:
                    status_color = {'good': '≡ƒƒó', 'warning': '≡ƒƒí', 'critical': '≡ƒö┤', 'moderate': '≡ƒƒá', 'info': '≡ƒö╡'}.get(rec['status'], 'ΓÜ¬')
                    st.markdown(f"**{status_color} {rec['category']}**: {rec['message']}")
                    st.markdown(f"   *≡ƒÆí {rec['suggestion']}*")
                    st.markdown("")
        ...
        with tab4:
            with st.container(border=True):
                st.markdown("<p class='card-title'>≡ƒôê Investment Strategy</p>", unsafe_allow_html=True)
        ...
        if cohort_recs['cohort_size'] > 0:
            ...
            st.markdown("<br>", unsafe_allow_html=True)
            st.info(f"Γ£¿ **Cohort Summary:** {cohort_recs['summary']}")
...
elif page == "≡ƒôê Data Visualization Dashboard":
    st.header("≡ƒôê Comprehensive Data Visualization Dashboard")
...
# ============ FINANCIAL TIPS & EDUCATION ============
elif page == "≡ƒôÜ Financial Tips & Education":
    st.header("≡ƒôÜ Financial Literacy & Education Center")
    st.info("Evidence-based financial education modules powered by behavioral insights.")
    
    with st.expander("≡ƒÆ░ The 50/30/20 Budget Rule", expanded=True):
        st.markdown("""
        **Needs (50%)** ΓÇö Housing, utilities, groceries, minimum debt payments, insurance.
        **Wants (30%)** ΓÇö Dining out, entertainment, subscriptions, hobbies.
        **Savings & Debt (20%)** ΓÇö Emergency fund, retirement, extra debt payments.
        """)
    
    with st.expander("≡ƒÜ¿ Emergency Fund Essentials"):
        st.markdown("""
    ...
    with st.expander("≡ƒöÑ FIRE Movement Basics"):
        st.markdown("""
        **Financial Independence, Retire Early** ΓÇö Save 50%+ of income to retire decades early.
        Rule of thumb: Multiply annual expenses by 25 (4% safe withdrawal rate).
        """)

# ============ DOCUMENT UPLOAD & STORAGE ============
elif page == "≡ƒùé∩╕Å Document Upload & Storage":
    st.header("≡ƒùé∩╕Å Secure Document Vault (Demo)")
    st.warning("ΓÜá∩╕Å This is a demo. In production, use encrypted storage and proper auth.")
    ...
    if uploaded_files:
        st.success(f"Uploaded {len(uploaded_files)} file(s) (demo only ΓÇö not persisted).")
        for f in uploaded_files:
            st.write(f"ΓÇó {f.name} ({f.size} bytes)")

# ============ EXPENSE CATEGORIZATION ============
elif page == "≡ƒÆ╕ Expense Categorization":
    st.header("≡ƒÆ╕ AI Expense Categorization")
...
elif page == "≡ƒö« Predictive Analytics":
    st.header("≡ƒö« Predictive Analytics Suite")
...
elif page == "≡ƒöö Alerts & Notifications":
    st.header("≡ƒöö Smart Alerts & Notifications")
...
# ============ PEER BENCHMARKING ============
elif page == "≡ƒæÑ Peer Benchmarking":
    st.header("≡ƒæÑ Peer Benchmarking")
...
# ============ SCENARIO SIMULATOR ============
elif page == "≡ƒö« Scenario Simulator":
    st.header("≡ƒö« What-If Scenario Simulator")
...
# ============ DATA EXPLORER ============
elif page == "≡ƒöì Data Explorer":
    st.header("≡ƒöì Raw Data Explorer")
