...
class UIComponents:
    ...
    def page_header(title: str, subtitle: str, icon: str = "📊") -> None:
        ...
        with col2:
            st.title(title)
            st.markdown(f"<p style='font-size: 1.1rem; color: #64748b; margin-top: -15px;'>{subtitle}</p>", unsafe_allow_html=True)
    ...
    @staticmethod
    def recommendations_tabs(recs: Dict[str, List[Dict]], tab_names: List[str] = None) -> None:
        ...
        for idx, tab in enumerate(tabs):
            with tab:
                with st.container(border=True):
                    st.markdown(f"<p class='card-title'>{tab_names[idx]}</p>", unsafe_allow_html=True)
    ...
    @staticmethod
    def sparkline(data: list, color: str = "#2563eb", height: int = 40) -> go.Figure:
    ...
    @staticmethod
    def gauge_chart(value: float, title: str, target: float = 90, height: int = 220) -> go.Figure:
                ...
                with col:
                    with st.container(border=False):
                        if isinstance(value, (int, float)) and not isinstance(value, bool):
                            st.metric(label, f"{value:,.1f}" if isinstance(value, float) else f"{value:,}")
                        else:
